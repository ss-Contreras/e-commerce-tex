...
//
// 1. Definición del esquema del formulario.
// Se usa un union para que el campo "image" acepte tanto un File (drag & drop)
// como un string (cuando se edita y se carga la URL existente).
//
...
export default function ProductsPage() {
  ...
  useEffect(() => {
    if (currentProduct) {
      // Al editar, asignamos la URL existente al campo "image"
      form.reset({ ...currentProduct, image: currentProduct.mainImageUrl });
    } else {
      form.reset();
    }
  }, [currentProduct, form]);
  ...
  //
  // Componente para drag & drop de imagen.
  // Usa react-dropzone para permitir seleccionar la imagen y genera un preview.
  //
  const ImageDropzone = ({
    value,
    onChange,
      ...
      <div>
        ...
        >
          ...
          {isDragActive ? (
            ...
            <p>Arrastra y suelta la imagen aquí, o haz clic para seleccionar el archivo</p>
          )}
        </div>
        {preview && (
          <img
